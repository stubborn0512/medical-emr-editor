...
function loadFonts(){
  ...
  fontLoadPromise=(async()=>{
    const jobs=FONTS.map(f=>new Promise(resolve=>{
...
function ensureModal(){
  ...
  m.querySelector('#sigApply').onclick=async()=>{const el=findSign();if(!el){alert('当前模板没有签名字段，请先选择亚健康复诊模板。');return}await loadFonts();const text=m.querySelector('#sigText').value.trim()||'示例医生';const family=m.querySelector('#sigFont').value;el.textContent='签名： '+text;el.classList.add('hand-sign-target');el.style.fontFamily='"'+family+'", "KaiTi", "STKaiti", cursive';el.style.fontSize=(+m.querySelector('#sigSize').value||28)+'px';el.style.transform='rotate('+(+m.querySelector('#sigAngle').value||0)+'deg)';el.style.letterSpacing=(+m.querySelector('#sigSpacing').value||0)+'px';el.dataset.handwritingSignature='1';if(document.fonts?.load)await document.fonts.load(`${el.style.fontSize} "${family}"`,text).catch(()=>[]);m.classList.remove('show');document.getElementById('saveState')&&(document.getElementById('saveState').textContent='● 已修改');};
  updatePreview();
}
function addButton(){
...
async function applyDefault(){
  ...
  await loadFonts();
  el.classList.add('hand-sign-target');el.style.fontFamily='"Ma Shan Zheng", "KaiTi", "STKaiti", cursive';el.style.fontSize='28px';el.style.transform='rotate(-2deg)';el.style.letterSpacing='0px';el.dataset.handwritingSignature='1';
  if(document.fonts?.load)await document.fonts.load('28px "Ma Shan Zheng"','示例医生').catch(()=>[]);
}
function boot(){addButton();applyDefault();}
